...
This pass has been built and tested using GCC 9.4.0 and 11.2.0 to compile it.
...
In the `tests` directory, you can use the `Makefile` to build our tests.f
...
The Python script `runTests.py` in the `tests` directory can be used to
repeatedly run the test cases both normally and under `valgrind`.  This script
is intended for testing the tests in the `array/`, `list/`, `tree/`,
`nested_array/`, and `nested_list/` directories and runs the `.exec` versions
and compares against the `.ref` version, reporting any differences.
